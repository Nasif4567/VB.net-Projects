...
Public Class MathUpload
    ...
    Private Sub SaveFileToDB(filePath As String)
        ...
        query = "Insert Into LearningMaterials(Topic,SubjectName,FileName,FileData,TeacherName,Teacher_ID) Values(@Topic,@SubjectName,@FileName,@FileData,@TeacherName,Teacher_ID)"
        cmd = New OleDbCommand(query, con)
        cmd.Parameters.Add("@Topic", OleDbType.VarWChar).Value = txtTopic.Text
        cmd.Parameters.Add("@SubjectName", OleDbType.VarWChar).Value = ms.Text
        cmd.Parameters.Add("@Filename", OleDbType.VarWChar).Value = fileName
        cmd.Parameters.Add("@FileData", OleDbType.VarBinary).Value = file.ToArray()
        cmd.Parameters.Add("@TeacherName", OleDbType.VarWChar).Value = Label3.Text
    ...
    Private Sub LoadData()
        ...
        Dim command As New OleDbCommand("select * from LearningMaterials where Teacher_ID =@ID", con)
    ...
    Private Sub btnmathup_Click(sender As Object, e As EventArgs) Handles btnmathup.Click
        ...
        End If
        If ms.Text = Nothing Or txtTopic.Text = Nothing Then
    ...
    Private Sub MathUpload_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        LoadData()
    End Sub
    ...
    Private Sub txtfileM_TextChanged(sender As Object, e As EventArgs) Handles txtfileM.TextChanged
